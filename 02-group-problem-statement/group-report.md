...
---
## 10. Tóm tắt giải pháp (Executive Solution Summary)

### 💡 Giải pháp là gì?
Giải pháp **Aggregator & Centralized Deadline Tracker** là một hệ thống gom và quản lý deadline tập trung, giúp người dùng tự động chuyển các thông báo rời rạc ở nhiều nền tảng thành một danh sách công việc rõ ràng, có ngữ cảnh và có đường link dẫn thẳng về tin nhắn gốc (*source link*).

### ⚙️ Cách giải quyết (Mô hình Workflow 3 lớp):
1. **Rule / API Sync (Lớp thu thập & lọc):** Tự động đồng bộ sự kiện từ Calendar và đọc dữ liệu (read-only) từ kênh được chọn. Dùng luật cố định để lọc bỏ các thông tin trùng lặp hoặc sự kiện đã có ngày giờ rõ ràng.
2. **AI / LLM (Lớp trích xuất ngôn ngữ):** Đóng vai trò "thư ký" đọc các tin nhắn chat tự nhiên (phi cấu trúc) trên Slack/Discord để nhặt ra các **Candidate Task** (gồm tên việc, hạn chót, tóm tắt nội dung và độ tin cậy).
3. **Human Boundary (Lớp phê duyệt của con người):** AI **không được tự ý tạo/sửa lịch hay gửi tin nhắn**. Người dùng dành 3–5 phút mỗi ngày xem qua danh sách Candidate Task để bấm **Approve (Duyệt) / Edit (Sửa) / Dismiss (Bỏ)**. Chỉ những task được duyệt mới chính thức ghi vào Calendar/Task Hub.

### 🚀 Chiến lược triển khai "Đi từ hẹp đến rộng" (Narrow-to-Wide Strategy):

* **Giai đoạn 1 — Thử nghiệm phạm vi hẹp (Pilot Scope):**
  * **Chỉ kết nối 2 nguồn:** Google Calendar (lịch chính) + 1 Kênh Slack/Discord thông báo cụ thể.
  * **Cách lấy dữ liệu:** Dùng API read-only hoặc nhập/copy văn bản thủ công trong vài ngày đầu để kiểm chứng độ chính xác của AI mà chưa tốn công tích hợp nhiều hệ thống phức tạp.
  * **Mục tiêu:** Chứng minh AI trích xuất chuẩn (Precision ≥90%, Recall ≥95%) và thực sự tiết kiệm thời gian (từ 15–30p xuống ≤8p/ngày) trước khi xin cấp thêm quyền.

* **Giai đoạn 2 — Mở rộng quy mô (Roadmap nâng cấp sang MVP):**
  * Sau khi Pilot chứng minh được hiệu quả và độ an toàn, hệ thống mới từng bước mở rộng kết nối các nền tảng còn lại qua các cổng tiêu chuẩn:
    * **LMS (Canvas/Moodle):** Đọc qua đường link đăng ký lịch `iCal (.ics)` công khai/cá nhân.
    * **Gmail:** Chỉ đọc các email được người dùng chủ động gắn nhãn (*Label*) bài tập/dự án.
    * **GitHub:** Kết nối API chỉ lấy Issue/PR được `@assigned` hoặc `@mentioned`.
    * **Notion:** Tích hợp API chỉ sync đúng 1 Database được chia sẻ quyền.
